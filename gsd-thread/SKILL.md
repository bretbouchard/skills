...
- Read frontmatter `status` field via:
  ```bash
  gsd-sdk frontmatter get .planning/threads/{file} status 2>/dev/null
  ```
...
2. Update the thread file's frontmatter `status` field to `resolved` and `updated` to today's ISO date:
   ```bash
   gsd-sdk frontmatter set .planning/threads/{SLUG}.md status resolved
   gsd-sdk frontmatter set .planning/threads/{SLUG}.md updated YYYY-MM-DD
   ```

3. Commit:
   ```bash
   gsd-sdk commit "docs: resolve thread — {SLUG}" ".planning/threads/{SLUG}.md"
   ```
...
```bash
gsd-sdk frontmatter set .planning/threads/{SLUG}.md status in_progress
gsd-sdk frontmatter set .planning/threads/{SLUG}.md updated YYYY-MM-DD
```
...
5. Commit:
   ```bash
   gsd-sdk commit "docs: create thread — ${ARGUMENTS}" ".planning/threads/${SLUG}.md"
   ```
...
- Status fields read via gsd-sdk frontmatter get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through gsd-sdk (or gsd-tools) which sanitizes input — keep that pattern
</security_notes>
